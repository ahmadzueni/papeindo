...
class CT_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        ...
        if($page_on_front) {
            ...
            $home_label = esc_html__( 'Home', 'intime' );
        }
        $args = wp_parse_args( $args, array(
            'home_label'            => $home_label,
            '404_label'             => esc_html__( 'Not Found', 'intime' ),
            'search_results_label'  => esc_html__( 'Search Results', 'intime' ),
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Tag:', 'intime' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'intime' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'intime_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'intime_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
         ...
         */
        $entries = apply_filters( 'intime_breadcrumb_single', $entries, $post );
